...
/*  新增                  document.getElementById获取控件传 */
   ...
   var chk_value=[];
   /* 依次遍历上下文所有的含有name=id的input..each就表示每一个       checked 是伪类选择器，用来对应被选择的单选框或者复选框 */
  	$('input[name="id"]:checked').each(function(){
  		alert($(this).val());
  		chk_value.push($(this).val());//push() 方法可向数组的末尾添加一个或多个元素
  	});
  	if(chk_value!=""){
		if(chk_value.toString().indexOf(",")>0){
		   alert("修改只能选择一条");
		}else{
			alert("选中的修改内容是"+chk_value);
...
//        dataType:"json",                                           // json 数据类型提交 
  ...
  <div class="container" style="height:630px;overflow-x:auto;">
    ...
    <div class="span5"><!-- 根据房间号模糊查询的一个div-->
	    <div class="row-fluid">
		    <label class="labelroomnumber">房间号：</label>
		    
		    <form action="${ctx}/RoomSet/tolist.do" method="post" style="float: left;">
			   <input id="txtnameid" name="roomNumber" class="textone roomnumberwidth" style="border-radius:0px; border-top-left-radius:4px; border-bottom-left-radius:4px;height:26px;" type="text" placeholder="请输入关键字" value="${roomNumber}">
...
<br>
    ...
    <div class="dgvone">
	      ...
	      <tbody id="tbody">
	        <c:forEach items="${list.result}" var="item"> 
		        ...
		        <tr>
		          ...
		          <td>${item.guestRoomLevelName}</td>         <!-- 客房等级ID -->
       ...
       <!-- 分页按钮  -->
   <p align="center" >
    	<a href="${ctx}/RoomSet/tolist.do?currentPage=1&tiaojian=${roomNumber}">首页</a>
    	<a href="${ctx}/RoomSet/tolist.do?currentPage=${list.currentPage-1}&roomNumber=${roomNumber}">上一页</a>
    	<a href="${ctx}/RoomSet/tolist.do?currentPage=${list.currentPage+1}&roomNumber=${roomNumber}">下一页</a>
    	<a href="${ctx}/RoomSet/tolist.do?currentPage=${list.totalPageCount}&roomNumber=${roomNumber}">尾一页</a>
    	<a>${list.currentPage}页/共${list.totalPageCount}页</a>
    </p>
