...
<html lang="en">
	...
	<body>
		...
		<script type="text/javascript">
			...
			/*  在以后动态切换到RTL模式时，我们保存了绘图函数和数据，以便重新绘制不同的位置，因此实际上不需要这样做。 */
			...
			function roomSet(){
				 ...
				 //第一句获取的是房间管理
			     var a=document.getElementById("roomManageId").getElementsByTagName('span')[0].innerHTML;//获取id为roomManageId下的span对象的内嵌内容
			     alert("获取到的内容是"+a);
			     //获取到啦也页面上的一个超链接，给这个超链接加上内容+房间管理
			     document.getElementById("bigGuide").innerHTML=a+"葛新海1";//获取id为bigGuide,就是一个在页面上的题标
			     
			     //获取的是客房设置
			     var s=document.getElementById("roomSetId").getElementsByTagName('span')[0].innerHTML;//roomSetId客房设置
			     
			     //获取到啦也页面上的一个超链接，给这个超链接加上内容+客房设置
			     document.getElementById("smallGuide").innerHTML=s+"葛新海2";
			...
			function commoditySet(){
			     ...
			     document.getElementById("smallGuide").innerHTML=s+"";
			     document.getElementById('Mainid').src='${ctx}/Commodity/tolist.do';  //控制器
			}
